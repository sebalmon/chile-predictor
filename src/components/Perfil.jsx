// src/components/Perfil.jsx  — v5 (Fase 2)
// ─────────────────────────────────────────────────────────────
// CAMBIOS v5:
//   • Sección "🔮 HISTORIAL DE PRONÓSTICOS" con HistorialPredicciones.
//   • Mantiene todo lo demás: cartas, puntosAyer, historial de días.
// ─────────────────────────────────────────────────────────────
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: "340px", width: "100%", textAlign: "center" }}
        ...
        <button className="btn-pixel btn-gris" onClick={onCerrar} style={{ fontSize: "7px" }}>
          CERRAR ✕
        </button>
      </div>
    </div>
...
export default function Perfil({ onVolver }) {
  ...
  // Historial de días (puntosDelDia)
  ...
  // Sección historial pronósticos
  const [mostrarHistPreds, setMostrarHistPreds] = useState(false);
  ...
  const cargarHistorialDias = async () => {
    ...
    <div style={{ padding: "16px 16px 80px" }}>
      ...
      <div className="perfil-header mb-16">
        <img
          ...
          style={{ width: "80px", height: "80px", imageRendering: "pixelated",
            border: "2px solid var(--verde-claro)", padding: "4px" }}
          ...
          {!editando ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "8px", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <button className="btn-pixel btn-verde" style={{ fontSize: "7px", padding: "6px 12px" }}
                  onClick={handleGuardarNick} disabled={guardando}>
                  {guardando ? "..." : "✅ OK"}
                </button>
                <button className="btn-pixel btn-gris" style={{ fontSize: "7px", padding: "6px 12px" }}
        ...
        <div style={{ display: "flex", gap: "16px", marginTop: "8px" }}>
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <span className="puntos-badge" style={{ fontSize: "12px" }}>
              {Object.values(userProfile?.cartas || {}).reduce((s, c) => s + c, 0)}
            </span>
          </div>
        </div>
      </div>

      {/* HISTORIAL DE PRONÓSTICOS (nuevo v5) */}
      <div className="seccion-titulo">🔮 HISTORIAL DE PRONÓSTICOS</div>
      <div className="caja-pixel mb-16" style={{ padding: 0, overflow: "hidden" }}>
        <button
          className="btn-pixel btn-gris w-full"
          style={{ fontSize: "7px", borderRadius: 0, border: "none",
            borderBottom: mostrarHistPreds ? "2px solid var(--verde-campo)" : "none" }}
          onClick={() => setMostrarHistPreds((v) => !v)}
        >
          {mostrarHistPreds ? "▲ OCULTAR" : "▼ VER MIS PRONÓSTICOS PASADOS"}
        </button>
        {mostrarHistPreds && (
          <HistorialPredicciones userId={firebaseUser?.uid} />
        )}
      </div>
      ...
      <div className="caja-pixel mb-16">
        {cargandoHist ? (
          ...
          <p style={{ fontSize: "7px", color: "var(--gris-claro)" }}>Sin actividad ayer.</p>
        )}
      </div>
      ...
      <div className="caja-pixel mb-16" style={{ padding: "8px" }}>
        {cargandoHist ? (
          ...
          <div style={{ maxHeight: "200px", overflowY: "auto" }}>
            {historialDias.map((h, i) => (
              <div key={i} style={{
                display: "flex", justifyContent: "space-between", alignItems: "center",
                padding: "6px 8px", borderBottom: "1px solid var(--verde-campo)",
              }}>
                <div>
        ...
        {CARTAS.map((carta) => {
          ...
          return (
            <div
              ...
              style={desbloq ? { cursor: "pointer", borderColor: RAREZA_COLOR[carta.rareza] || "var(--negro)" } : {}}
              ...
              {desbloq ? (
                <>
                  ...
                  <span style={{ fontFamily: "'Press Start 2P',monospace", fontSize: "6px",
                    color: "var(--gris-claro)", marginLeft: "4px" }}>
                    (x{cantidad})
                  </span>
                </>
              ) : (
