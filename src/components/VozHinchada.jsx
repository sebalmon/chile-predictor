...
// ── Reacciones con popover ────────────────────────────────────
function Reacciones({ docId, miUid }) {
  const [conteo,     setConteo]     = React.useState({});
  const [miReaccion, setMiReaccion] = React.useState(null);
  const [abierto,    setAbierto]    = React.useState(false);
  const popRef = React.useRef(null);

  React.useEffect(() => { if (docId) cargar(); }, [docId]);

  React.useEffect(() => {
    if (!abierto) return;
    const h = (e) => { if (popRef.current && !popRef.current.contains(e.target)) setAbierto(false); };
  ...
  const reaccionar = async (emoji) => {
    ...
    if (miReaccion === emoji) {
      await deleteDoc(ref);
      setConteo(prev => { const n={...prev}; n[emoji]=Math.max(0,(n[emoji]||0)-1); if(!n[emoji])delete n[emoji]; return n; });
      setMiReaccion(null);
    } else {
      if (miReaccion) setConteo(prev => { const n={...prev}; n[miReaccion]=Math.max(0,(n[miReaccion]||0)-1); if(!n[miReaccion])delete n[miReaccion]; return n; });
    ...
    <div style={{ marginTop:"5px",display:"flex",alignItems:"center",gap:"4px",flexWrap:"wrap" }}>
      <div ref={popRef} style={{ position:"relative" }}>
        <button onClick={() => setAbierto(v=>!v)}
          ...
          style={{ fontSize:"13px",padding:"2px 5px",cursor:"pointer",
            background:miReaccion?"rgba(255,255,255,0.22)":"rgba(255,255,255,0.07)",
            border:miReaccion?"1px solid rgba(255,255,255,0.45)":"1px solid rgba(255,255,255,0.15)",
            borderRadius:"3px",lineHeight:1 }}>
          {miReaccion || "😀"}
        </button>
        {abierto && (
          <div style={{ position:"absolute",bottom:"calc(100% + 4px)",left:0,
            background:"var(--negro)",border:"2px solid var(--verde-campo)",
            boxShadow:"3px 3px 0 rgba(0,0,0,0.5)",padding:"5px 6px",
            display:"flex",gap:"2px",zIndex:100,borderRadius:"2px" }}>
            {EMOJIS.map(e => (
              <button key={e} onClick={() => reaccionar(e)}
                style={{ fontSize:"15px",padding:"3px 4px",cursor:"pointer",
                  background:miReaccion===e?"rgba(255,255,255,0.22)":"transparent",
      ...
      {tieneReacciones && Object.entries(conteo).filter(([,n])=>n>0).map(([e,n]) => (
        <span key={e} style={{ fontSize:"9px",background:"rgba(0,0,0,0.2)",
          padding:"1px 5px",border:"1px solid rgba(255,255,255,0.1)",borderRadius:"2px" }}>
          {e} {n}
        </span>
      ))}
    </div>
